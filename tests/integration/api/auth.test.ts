...
describe('Auth API Integration', () => {
  ...
  it('POST /api/auth/register should reject duplicate email', async () => {
    ...
    await db.run(
      "INSERT INTO users (id, user_uid, email, password_hash, role, level, is_active, email_verified) VALUES (?, ?, ?, ?, 'user', 0, 1, 0)",
      [uuidv4(), 1, 'duplicate@test.com', passwordHash]
    );
  ...
  it('POST /api/auth/login should login successfully', async () => {
    ...
    await db.run(
      "INSERT INTO users (id, user_uid, email, password_hash, role, level, is_active, email_verified) VALUES (?, ?, ?, ?, 'user', 0, 1, 0)",
      [userId, 1, 'login@test.com', passwordHash]
    );
  ...
  it('POST /api/auth/login should reject wrong password', async () => {
    ...
    await db.run(
      "INSERT INTO users (id, user_uid, email, password_hash, role, level, is_active, email_verified) VALUES (?, ?, ?, ?, 'user', 0, 1, 0)",
      [userId, 1, 'wrongpw@test.com', passwordHash]
    );
  ...
  it('GET /api/auth/me should return current user info', async () => {
    ...
    await db.run(
      "INSERT INTO users (id, user_uid, email, password_hash, role, level, is_active, email_verified) VALUES (?, ?, ?, ?, 'user', 0, 1, 0)",
      [userId, 1, 'me@test.com', passwordHash]
    );

    const profileId = uuidv4();
    await db.run(
      "INSERT INTO profiles (id, user_id, name) VALUES (?, ?, ?)",
      [profileId, userId, 'MeUser']
    );

    const accessToken = 'test-access-token-me';
