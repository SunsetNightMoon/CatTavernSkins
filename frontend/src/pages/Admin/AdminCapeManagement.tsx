...
export default function AdminCapeManagement() {
  ...
  const loadCapes = async (p: number = page) => {
    ...
    try {
      ...
      message.error(`加载披风列表失败: ${e.message}`)
      console.error('加载披风列表失败:', e)
    } finally {
      setLoading(false)
    }
  }
    ...
    },
    {
      ...
      render: (_: any, r: Cape) => (
        <Space>
          ...
          <Popconfirm title="确认删除此披风？" onConfirm={() => handleDelete(r)} okText="确认" cancelText="取消">
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--text-muted)', marginBottom: 16, fontSize: 13 }}>
        共 {total || capes.length} 个披风，以下是所有用户上传的披风。
      </p>
      <Table
        columns={columns}
        dataSource={capes}
      ...
      >
        <Form form={editForm} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item name="name" label="披风名称" rules={[{ required: true, message: '请输入名称' }]}>
            <Input placeholder="披风名称" maxLength={50} />
          </Form.Item>
          <Form.Item name="description" label="描述">
            <Input.TextArea rows={3} placeholder="披风描述（可选）" maxLength={200} />
          </Form.Item>
