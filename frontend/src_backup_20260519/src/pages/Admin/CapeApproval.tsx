...
function CapeApproval() {
  ...
  const loadPendingCapes = async () => {
    ...
    try {
      ...
      message.error('加载披风列表失败')
    } finally {
      setLoading(false)
    }
  }

  const handleApprove = async (capeId: number) => {
    try {
      await fetch(`/api/admin/capes/${capeId}/approve`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleReject = (capeId: number) => {
    Modal.confirm({
      title: '拒绝披风',
      content: '确定要拒绝这个披风吗？',
      onOk: async () => {
        try {
          await fetch(`/api/admin/capes/${capeId}/reject`, {
            method: 'PUT',
            headers: {
              'Authorization': `Bearer ${token}`,
            },
    ...
    },
    {
      ...
      render: (_, record: Cape) => (
        <img
          ...
          alt="Cape"
          style={{ width: 50, height: 'auto', border: '1px solid #d9d9d9' }}
        />
      ),
    },
    {
