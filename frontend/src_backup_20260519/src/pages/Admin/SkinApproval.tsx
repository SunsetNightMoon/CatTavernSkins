...
function SkinApproval() {
  ...
  const handleApprove = async (skinId: number) => {
    try {
      await fetch(`/api/admin/skins/${skinId}/approve`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleReject = (skinId: number) => {
    Modal.confirm({
      title: '拒绝皮肤',
      content: '确定要拒绝这个皮肤吗？',
      onOk: async () => {
        try {
          await fetch(`/api/admin/skins/${skinId}/reject`, {
            method: 'PUT',
            headers: {
              'Authorization': `Bearer ${token}`,
            },
    ...
    },
    {
      ...
      render: (_, record: Skin) => (
        <img 
          ...
          alt="Skin" 
          style={{ width: 50, height: 25, border: '1px solid #d9d9d9' }}
        />
      ),
    },
    {
